...
long scull_ioctl(struct file *filp, unsigned int cmd, unsigned long arg) {
    ...
    CHECK(_IOC_TYPE(cmd) == SCULL_IOC_MAGIC, -ENOTTY, "ioc type was not SCULL_IOC_MAGIC. Type was %d.", _IOC_TYPE(cmd));
    CHECK(_IOC_NR(cmd) <= SCULL_IOC_MAXNR, -ENOTTY, "ioc number was %d, which is > SCULL_IOC_MAXNR (%d).", _IOC_NR(cmd), SCULL_IOC_MAXNR);

    // The direction is a bitmask. VERIFY_WRITE catches r/w transfers. 'Type' is user-oriented,
    // while access_ok is kernel-oriented, so the concept of "read" and "write" is reversed.
    // TODO make this prettier once we understand it better.
    if(_IOC_DIR(cmd) & _IOC_READ) {
        err = !access_ok_wrapper(VERIFY_WRITE, (void __user *)arg, _IOC_SIZE(cmd));
    } else if(_IOC_DIR(cmd) & _IOC_WRITE) {
        err = !access_ok_wrapper(VERIFY_READ, (void __user *)arg, _IOC_SIZE(cmd));
    }
    CHECK(err == 0, -EFAULT, "access_ok_wrapper failed.");
